...
| `0.2.4` | Executable reducer and memory contract | A complete internal state witness is distinct from redacted observations; disjoint arenas and infallible-commit/epoch-loss rules remain executable |
...
| `0.17.4` | Bounded cryptographic key leases | Concurrent and prepared use of opaque keys is purpose/provider/generation/expiry-bound, non-clone, and revocation-aware |
...
| `0.21.1` | Terminal stream-framing failures | Impossible prefixes, bad padding, oversize declarations, and partial EOF close without resynchronization or superlinear work |
...
| `0.23.18` | Unwind engine-epoch poisoning | One outer `Idle -> InCall -> Idle|Poisoned` guard rejects reentrancy and cannot be disarmed by nested providers, while production still aborts |
...
| `0.30.10` | Typed canonical ingress envelope | Sealed transport provenance promotes through raw, validated, and complete states; only complete ingress reaches classification and preserves authorized response-source selection |
...
| `0.37.5` | Execution-time endpoint authority | Runtime checks only adapter-local facts and acknowledged fences; semantic freshness remains core capability authority and uncertain mirrors fail closed |
